...
exports.createPostLike = async (req, res) => {
    ...
    try {
        ...
        const [{ author_id }] = await Post.getPostById(postId);
        await User.ratingPlus(author_id);
...
exports.createCommentLike = async (req, res) => {
    ...
    try {
        ...
        const [{ author_id }] = await Comment.getCommentById(commentId);
        await User.ratingPlus(author_id);
        response.status(200, { message: "Like Added" }, res);
    }
    catch (e) {
...
exports.deleteLikePost = async (req, res) => {
    ...
    const [{ author_id }] = await Post.getPostById(id);
    await User.minusRating(author_id);
